...
namespace BossPatterns {
    ...
    public static class BossHelpers {
        ...
        public static void RotateAroundCenter(Transform target, float angleDeg, Transform centerPoint)
        {
            ...
            // 회전 적용
            Quaternion targetRotation = centerPoint.rotation * Quaternion.Euler(0, 0, angleDeg);

            // 세계 좌표로 변환 후 설정
            Vector3 newPosition = new Vector3(
                centerPoint.TransformDirection(localPos).x,
                centerPoint.TransformDirection(localPos).y,
                localPos.z
            );
        ...
        #region Pattern A Helper Methods
        ...
        public static bool IsRhythmTiming(float currentTime, float[] rhythmSequence)
        {
            ...
            {
                if (Mathf.Abs(currentTime - rhythmSequence[i]) < 0.1f)
                {
        ...
        #region Pattern B Helper Methods
        ...
        public static bool CheckJumpInput(bool isJumping)
        {
            ...
            // 추가 조건: 점프 범위 확인
            PlayerController player = FindObjectOfType<Rigidbody2D>();
        ...
        #region Pattern D Helper Methods
