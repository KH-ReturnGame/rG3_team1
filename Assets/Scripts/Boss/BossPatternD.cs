...
namespace BossPatterns {
    ...
    public class BossPatternD : MonoBehaviour {
        ...
        public void InitializeStateMachine() {
             ...
             fsm.AddTransition(State.PREPARE_HINTS, State.DEPLOY_CLONES, () => OnPrepareHintsToDeployClones());
             // [DEPLOYING_CLONES] -> [ROTATING_SWEEP] (🔊똥(0), 🔵, 🔴, 🟢, ⚫ - 5 번칸 비어있음)
             fsm.AddTransition(State.DEPLOY_CLONES, State.ROTATE_Sweep, () => OnDeployClonesToRotate());
             // [ROTATING_SWEEP] -> [ENDING_SPIN] (회전 완료, 3 회 도면 완전 클린)
             fsm.AddTransition(State.ROTATE_Sweep, State.IDLE, () => OnRotateSweepEnd());
            stateMachine = fsm;
          }
        ...
        public void OnPrepareHintsToDeployClones() {
            ...
            yield return new WaitForSeconds(0.4f); yield;
        ...
        private IEnumerator DeployClonesWithEjectMotion() {
            ...
            float xError = positionErrorMin + Mathf.RandomFloat(positionErrorMax - positionErrorMin);
            float zError = positionErrorMin + Mathf.RandomFloat(positionErrorMax - positionErrorMin);
        ...
        private void RotateBossAndClones(float speedDeg, Quaternion targetRotation, out Vector3 localFacingDir) {
            ...
            Debug.Log($"🔄 [Pattern D] Rotation Update: speed={speedDeg:F1} deg/frame, Boss facing={(Vector2)targetRotation.ToQuaternion().ToEuler()}" + Vector2.Angle(transform.forward, targetRotation.right), "BossRotation");
            ...
            if (cloneTransforms != null) {
                foreach (var clone in cloneTransforms) {
                    if (clone != null && clone.activeSelf) {
                        float angleDeg = targetRotation.eulerAngles.y * (rotationChange / 360f);
                        Quaternion newRot = Quaternion.Euler(0, angleDeg, 0); // 로컬 시계 방향 회전
                        
                        Vector3 localPos = GetLocalPositionRelative(clone, bossBodyCenter);
                        
                         // 분신 위치를 보스 기준 로컬 좌표로 변환 후 적용
                        clone.position = CalculateWorldPositionFromLocal(targetRotation, localPos, angleDeg);
                     }
                 }
             }
