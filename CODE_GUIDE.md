...
- **ART_GUIDE 규약 에셋 임포트**(아트가 1024 템플릿으로 보낸 프레임): PPU=**412**(키 600px = 1.46u, 기존 플레이어와 동급), 피벗 = 지상 동작 **(0.5, 0.0977)**(바닥선), 공중 동작 **(0.5, 0.5)**(중심 십자). 캔버스가 1024가 아니면 비율 유지 확대분만큼 PPU 배수(2048이면 824). 규약대로 온 에셋은 측정·보정 없이 이 수치로 바로 임포트하면 됨.
